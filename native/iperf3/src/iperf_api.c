...
void
iperf_interrupt(struct iperf_test *ipt)
{
    char byte = 1;
    ...
    if (ipt->interrupt_write_fd != -1) {
        (void) write(ipt->interrupt_write_fd, &byte, sizeof(byte));
    }
}
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    testp->interrupt_read_fd = -1;
    testp->interrupt_write_fd = -1;
...
iperf_free_test(struct iperf_test *test)
{
    ...
    if (test->interrupt_read_fd != -1) {
        close(test->interrupt_read_fd);
        test->interrupt_read_fd = -1;
    }
    if (test->interrupt_write_fd != -1) {
        close(test->interrupt_write_fd);
        test->interrupt_write_fd = -1;
    }
