...
iperf_server_listen(struct iperf_test *test)
{
    ...
    if (test->interrupt_read_fd == -1 && pipe(interrupt_fds) == 0) {
        test->interrupt_read_fd = interrupt_fds[0];
        test->interrupt_write_fd = interrupt_fds[1];
    }
    if (test->interrupt_read_fd != -1) {
        FD_SET(test->interrupt_read_fd, &test->read_set);
        if (test->interrupt_read_fd > test->max_fd) test->max_fd = test->interrupt_read_fd;
    }
...
iperf_run_server(struct iperf_test *test)
{
	...
	if (result > 0) {
            if (test->interrupt_read_fd != -1 && FD_ISSET(test->interrupt_read_fd, &read_set)) {
                char buffer[16];
                (void) read(test->interrupt_read_fd, buffer, sizeof(buffer));
                iperf_set_test_state(test, IPERF_DONE);
                continue;
            }
